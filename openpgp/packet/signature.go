...
// HashUserId computes and returns the hash digest of a UserID message. This
// can later be fed to a hardware token that outputs the signature data that in
// turn can be fed to SetSigData.
func (sig *Signature) HashUserId(id string, pub *PublicKey) ([]byte, error) {
	h, err := userIdSignatureHash(id, pub, sig.Hash)
	if err != nil {
		return nil, err
	}

	// Below is the same as in func (sig *Signature) Sign(), modified not to use a privkey.
	...
	return sig.signPrepareHash(h)
}

// HashSubkey computes and returns the hash digest of a subkey message. This
// can later be fed to a hardware token that outputs the signature data that in
// turn can be fed to SetSigData.
func (sig *Signature) HashSubkey(pub *PublicKey, issuerPub *PublicKey) ([]byte, error) {
	h, err := keySignatureHash(issuerPub, pub, sig.Hash)
	if err != nil {
		return nil, err
	}
	...
	// Below is the same as in func (sig *Signature) Sign(), modified not to use a privkey.
	sig.Version = pub.Version
	sig.IssuerFingerprint = issuerPub.Fingerprint
	sig.outSubpackets, err = sig.buildSubpackets(*pub)
	if err != nil {
		return nil, err
	}
...
// HashRevocation computes and returns the hash digest of a revocation message. This
// can later be fed to a hardware token that outputs the signature data that in
// turn can be fed to SetSigData.
func (sig *Signature) HashRevocation(pub *PublicKey) ([]byte, error) {
...
// SetSigData modifies the signature data with given bytes. This can be used to
// generate signature packets when the signature data is generated by a
// hardware token. Call Serialize to write it out.
func (sig *Signature) SetSigData(pka PublicKeyAlgorithm, sigdata []byte) (err error) {
	switch sig.PubKeyAlgo {
	case PubKeyAlgoEdDSA:
		if len(sigdata) != 64 {
			err = errors.StructuralError("expected 64 bytes for EdDSA signature data")
		}
		sig.EdDSASigR = encoding.NewMPI(sigdata[:32])
		sig.EdDSASigS = encoding.NewMPI(sigdata[32:])
	default:
